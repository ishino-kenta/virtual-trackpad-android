...
1〜4 本指のジェスチャーで、カーソル操作・スクロール・ピンチズーム・進む/戻るをワイヤレスで送信できます。
...
- 接続が想定外に切れた場合の自動再登録
- 前回ペアリング先への自動再接続
- 視覚フィードバック（指のドット、ピンチの目盛り線、4 本指スワイプの予兆カーテン）
...
- **3 本指ドラッグ** : 左ボタン押下中のドラッグ (範囲選択・ウィンドウ移動など)
...
### 拡張ジェスチャー
- **4 本指スワイプ (左右)** : `Alt + ←` / `Alt + →`（ブラウザの戻る/進む）
- スワイプ予兆の「カーテン UI」が反対側からせり出し、しきい値到達で発火
- **4 本指ダブルタップ** : 画面を完全ブラックアウト (電池消費を抑える)。もう一度同じジェスチャで復帰
...
| 4 本指スワイプ (左→右) | 進む (`Alt + →`) |
| 4 本指スワイプ (右→左) | 戻る (`Alt + ←`) |
| 4 本指ダブルタップ | 画面ブラックアウト ON/OFF (省電力用) |
| 右上ドット長押し | 設定画面を開く |
| 右上ドット 3 連タップ | ログ画面を開く |
...
画面右上のドットを **3 回連続でタップ** するとログ画面に入ります。
HID イベントや接続状態の履歴を確認できる、うまく繋がらない時のデバッグ用画面です。
...
本プロジェクトの開発には AI コーディングアシスタントを活用しています。
